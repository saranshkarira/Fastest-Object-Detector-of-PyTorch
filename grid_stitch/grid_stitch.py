...
def scale_crop(images, scale):
    for i in range(len(images)):
        ...
        # print im.shape
        if new_h != scale:
            top = np.random.randint(0, new_h - scale)
        else:
            top = 0
        if new_w != scale:
            left = np.random.randint(0, new_w - scale)
        else:
            left = 0

        im = im[top: top + scale, left: left + scale]
        print im.shape
        images[i] = im
...
def grid_stitch(scale, grid, p1, p2):
    ...
    # print(len(s_images))
    # print(s_images)
    s_images = scale_crop(s_images, scale)
    g_squared = grid * grid
    for i in range(0, len(s_images) - g_squared, g_squared):
        # print(s_images)
        g = i + g_squared
        # print(type(i), g, scale, grid, 'this')
        imgs = np.asarray(s_images[i:g]).reshape(grid, grid, scale, scale, 3)
        hstacks = [np.hstack(imgs[i, :, :, :, :]) for i in range(grid)]
        new = np.vstack(hstacks)
...
with env.begin(write=False) as txn:
    with txn.cursor() as cursor:
        for i in range(len(imagenames)):
            images.append(cv2.imdecode(np.fromstring(cursor.get(imagenames[i].encode()), dtype=np.uint8), 1))

grid_stitch(320, 2, 25, 50)
